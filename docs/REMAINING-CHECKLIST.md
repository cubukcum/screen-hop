...
actuation + calibration + reconcile) is wired (152 tests pass; `cargo fmt`, `cargo clippy
...
- 🟡 **GUI onboarding wizard** — **Step 1 (Pair) is done + honest**: first run with no secret opens the
  wizard; typing a shared passphrase + Pair saves the `mesh-secret` (same format as the CLI/file) and
  relaunches straight into the live mesh — no hand-created file needed. Step 1 was also redrawn to the
  ...
  Step 3 calibrate matrix, Step 4 rename→labels. Follow-ups: Phase 2 (Step 2 real probe), Phase 3
...
- ✅ **CI** ([.github/workflows/ci.yml](../.github/workflows/ci.yml)): fmt + clippy + build + test on `windows-latest` (current stable), plus an **installer** job (Inno Setup via choco → `screen-hop-setup.exe` + SHA-256) — the **one** user-facing download; the installer bundles `screenhop-ui.exe` and the spike tool inside. (Dropped: the pinned MSRV-1.82 job — Slint requires rustc 1.92 — and the loose-binaries job, which confused testers with three unlabeled `.exe`s on the release page.)
